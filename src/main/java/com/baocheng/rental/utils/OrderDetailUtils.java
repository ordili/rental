...
import com.baocheng.rental.controller.RentalCarController;
import com.baocheng.rental.model.entity.Car;
import com.baocheng.rental.model.order.BackOrderDetail;
import com.baocheng.rental.model.order.OrderDetail;
...
public class OrderDetailUtils {
    ...
    public static void checkRentalOrderAndSetState(RentalOrderDetail rentalOrderDetail) {
        ...
        if (rentalOrderDetail == null) {
            rentalOrderDetail.setState(OrderDetailState.ORDER_DETAIL_IS_NULL);
            return;
        }
        if (rentalOrderDetail.getCar() == null) {
            rentalOrderDetail.setState(OrderDetailState.CAR_IS_NULL);
            return;
        }

        Long orderQuantity = rentalOrderDetail.getOrderQuantity();
        if (orderQuantity == null || orderQuantity.compareTo(1L) < 0) {
            rentalOrderDetail.setState(OrderDetailState.ORDER_QUANTITY_ILLEGAL);
            return;
        }
        BigDecimal dailyRent = rentalOrderDetail.getDailyRent();
        if (dailyRent == null || dailyRent.compareTo(BigDecimal.ZERO) <= 0) {
            rentalOrderDetail.setState(OrderDetailState.DAILY_RENT_ILLEGAL);
            return;
        }

        if (!checkFromDayAndToDayIsLegal(rentalOrderDetail)) {
            rentalOrderDetail.setState(OrderDetailState.TENANCY_ILLEGAL);
            return;
        }
    ...
    public static void checkBackOrderAndSetState(BackOrderDetail backOrderDetail) {
        ...
        if (backOrderDetail == null) {
            backOrderDetail.setState(OrderDetailState.ORDER_DETAIL_IS_NULL);
            return;
        }
        if (backOrderDetail.getCar() == null) {
            backOrderDetail.setState(OrderDetailState.CAR_IS_NULL);
            return;
        }

        Long orderQuantity = backOrderDetail.getOrderQuantity();
        if (orderQuantity == null || orderQuantity.compareTo(1L) < 0) {
            backOrderDetail.setState(OrderDetailState.ORDER_QUANTITY_ILLEGAL);
            return;
        }
    ...
    private static boolean orderDetailIsValid(RentalOrderDetail rentalOrderDetail) {
        return rentalOrderDetail != null && rentalOrderDetail.getState() == OrderDetailState.NORMAL && rentalOrderDetail.getActualRentalQuantity() > 0L;
    }
